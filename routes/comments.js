...
//Edit comment
router.get("/:comment_id/edit", middlewares.checkCommentOwnership, function(req, res) {
...
router.delete("/:comment_id", middlewares.checkCommentOwnership, function(req, res) {
    Comment.findByIdAndRemove(req.params.comment_id, function(err, updateComment){
        if(err){
            res.redirect("back");
        }else{
